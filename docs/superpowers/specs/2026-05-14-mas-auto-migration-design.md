...
5. The page swaps the form for a success card:
   ...
   > **[Continue to checkout]** (opens the Paddle pay link)
6. Customer completes Paddle checkout. Paddle emails the license to the email it has on file. Done.
...
The form page. Static. Embeds the Turnstile widget via `<script src="https://challenges.cloudflare.com/turnstile/v0/api.js" async defer>` and renders the drop zone. Imports `src/scripts/upgrade.ts` once.

### `src/scripts/upgrade.ts`

Client-side TypeScript. Wires up drag-and-drop, reads the Turnstile token from the widget, builds the `FormData`, posts to `/api/upgrade-request`, and replaces the form with the success or error card based on the JSON response. No framework — same vanilla style as `site.ts`.
